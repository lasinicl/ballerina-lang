...
function createOrderByFunction(function(_Frame _frame) orderFunc, boolean[] orderDirections)
        returns _StreamFunction {
    return new _OrderByFunction(orderFunc, orderDirections);
}
...
function sortStreamFunction(stream<Type, error?> strm, @tainted Type[] arr) returns stream<Type, error?> {
