...
type _OrderByFunction object {
    *_StreamFunction;
    ...
    function init(function(_Frame _frame) orderFunc, boolean[] orderDirections) {
        self.orderFunc = orderFunc;
        orderDirectionsArr = orderDirections;
        self.prevFunc = ();
    }
    ...
    public function process() returns _Frame|error? {
        ...
        if (pFrame is _Frame) {
            f(pFrame);
            anydata[] v = <anydata[]> pFrame["$orderKey$"];
...
type StreamOrderBy object {
    ...
    public function topDownMergeSort(@tainted Type[] strmEvents) returns @tainted Type[] {
        anydata[][] sortFieldsClone = self.sortFields.clone();
        self.topDownSplitMerge(0, self.sortFields.length(), self.sortFields, strmEvents);
        return strmEvents;
    }

    function topDownSplitMerge(int iBegin, int iEnd, @tainted anydata[][] a, Type[] strmEvents) {
        if (iEnd - iBegin < 2) {
            return;
        }
        int iMiddle = (iEnd + iBegin) / 2;
        self.topDownSplitMerge(iBegin, iMiddle, a, strmEvents);
        self.topDownSplitMerge(iMiddle, iEnd, a, strmEvents);
        self.topDownMerge(iBegin, iMiddle, iEnd, a, strmEvents);
    }

    function topDownMerge(int iBegin, int iMiddle, int iEnd, @tainted anydata[][] a, Type[] strmEvents) {
        int index = 0;
        int i = iBegin;
        int j = iMiddle;
        int k = iBegin;
        int len = strmEvents.length();
        Type[] strmEventsClone = [];

        while (index < len) {
            strmEventsClone[index] = strmEvents[index];
            index += 1;
        }

        while (k < iEnd) {
            if (i < iMiddle && (j >= iEnd || self.sortFunc(a[i], a[j], 0) < 0)) {
                strmEvents[k] = strmEventsClone[i];
                i = i + 1;
            } else {
                strmEvents[k] = strmEventsClone[j];
                j = j + 1;
            }
            k += 1;
        }
    ...
    function sortFunc(anydata[] x, anydata[] y, int i) returns @tainted int {
        if (x[i] is string) {
            ...
            if (y[i] is (int|float|decimal)) {
                int c;
                if (self.sortTypes[i]) {
                    c = self.numberSort(<int|float|decimal>x[i], <int|float|decimal>y[i]);
                } else {
                    c = self.numberSort(<int|float|decimal>y[i], <int|float|decimal>x[i]);
                }
                return self.callNextSortFunc(x, y, c, i + 1);
            } else {
    ...
    function callNextSortFunc(anydata[] x, anydata[] y, int c, int i) returns @tainted int {
        int result = c;
        if (result == 0 && (self.sortTypes.length() > i)) {
